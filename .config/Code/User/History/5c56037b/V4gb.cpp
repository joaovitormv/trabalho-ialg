...
void listar(banda vetor[]){
	int tam = sizeof(vetor);
	for(int i =0; i<tam; i++){
		cout<<vetor[i].nome<<" "<<vetor[i].numAlbuns<<" "<<vetor[i].numVendas<<" milhoes "<<vetor[i].numMembrosFund<<" "<<vetor[i].genero<<"HHHHH"<<endl;
		}
	};


void ordenarPorAlbuns(){
...
int main(){
    ...
    qtd_bandas = lerBandasDoCSV("bandas.csv", bandas);
    do{
        menu();
        cin>>op;
        switch(op){
