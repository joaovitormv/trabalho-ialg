...
int lerBandasDoCSV(const char* nomeArquivo, banda bandas[] ) {
    ...
    getline(arquivocsv, linha);
     ...
     while (i < maxBandas && getline(arquivocsv, linha)) {
        ...
        stringstream ss(linha);Redimensionar
...
void buscarPorNumAlbuns(banda vet[], int pesquisa, int qtd_bandas){
    ordenarPorAlbuns(vet, qtd_bandas);
    int i, posicao = -1;
    int pos_inicial = 0;
    ...
    while (pos_inicial <= pos_final) {
  	    ...
  	    if (pesquisa == vet[meio].numAlbuns){
     	   posicao = meio;
...
19:35		Está em Atividade(booleano);
Número de Vendas(float);
Número de Membros Fundadores(int);
Gênero(String);, bandas);
    do{
        menu();
        cin>>op;
        switch(op){
